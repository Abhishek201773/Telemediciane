...
def patient_home(request):
    ...
    if not user_profile:
        context = {'profile_status': 'Please Create Profile To Continue', 'doctor': doctor, 'ment': appointment,
                   patient: 'patient', 'drug': medical3}
...
def MakePredict(request):
    ...
    list_a = ['itching', 'skin_rash', 'nodal_skin_eruptions', 'continuous_sneezing', 'shivering', 'chills',
              'joint_pain', 'stomach_pain', 'acidity', 'ulcers_on_tongue', 'muscle_wasting', 'vomiting',
    ...
    clf = joblib.load('model/naive.pkl')
...
def MakeMend(request):
    ...
    disease_list = ['Acne', 'Allergy', 'Diabetes', 'Fungal infection', 'Urinary tract infection', 'Malaria', 'Malaria',
                    'Migraine', 'Hepatitis B', 'AIDS']
    ...
    if sick in disease_list:
        ...
        clf = joblib.load('model/Med_GNB.pkl')
        prediction = clf.predict(test)
